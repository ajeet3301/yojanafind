...
// ===== CRON SCHEDULER (every 7 days at 2 AM) =====
cron.schedule('0 2 */7 * *', () => {
  log('info', 'Cron triggered: starting weekly crawl');
...
app.listen(PORT, () => {
  ...
  log('info', 'Cron schedule: every 7 days at 2:00 AM');
  log('info', `Monitoring ${GOVT_SOURCES.length} government sources`);
  
  // Run first crawl 30s after startup if no data exists
  setTimeout(async () => {
